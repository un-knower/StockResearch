...
public class StockSelStrag {
	...
	// 白名单股票   
	public final static String whStock = "600593,002001,002202,0000001,1399001,002460,603799,603993,002497,002460,600703";
	...
	public static List<EastReportBean> queryEarningsStock() {
		...
		query.mustNot(QueryBuilders.inQuery("stockCode",
				(StockExpCode.execBLCKStockDefined + "," + excStockBchipStock + "," + excGrowUpStock + execRepStock).split(",")));
	...
	public static Map<String, String> blckLstOfStock() {
		...
		for (EastReportBean bean : lstResult) {
		 ...
		 for (int i = 0; i < StockExpCode.execBLCKStockDefined.split(",").length; i++) {
				mapsInfo.put(StockExpCode.execBLCKStockDefined.split(",")[i], STRA_TYPE_DEF);// 特殊之前接触过的坑股
		}
	...
	public static void main(String[] args) {
		System.out.println(getSubnewStock(-200).keySet());
