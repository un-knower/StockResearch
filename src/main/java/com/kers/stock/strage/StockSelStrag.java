...
public class StockSelStrag {
	...
	public final static String execStockDefined = "002822,000851,601928,600548,600545,600582,600895,600480,002295,600557,603701,600626,000838,000890,002686,000790,600639,000066,600737,600814,603901,000976,002776,600312,002541,002331,600528,002014,601858,603128,600838,603508,601633,600732,601519,600225,002650,002832,603158,600853,002059,603908,600317,600578,000027,000539,002498,002641,603606,601618,600023,600488,601866,600863,601018,601872,600642,601339,600033,600811,002239,600170,600220,000420,600635,002266,600665,600163,600620,000421,002375,000958,000987,002748,603985,600415,600372,000514,002574,002187,600865,603618,002567,600052,603658,601518,000157,002561,600765,000631,000637,600461,603313,600758,002889,000753,603050,600292,000591,601718,600272,600135,002367,603268,002364,600007,601880,601238,601390,000617,000166,600010,600641,600020,000921,600633,600688,000541,300495,002468,002323,002925,601828,001965,300433";
	...
	public static List<StockBaseInfo> queryGrowUpStock() {
		...
		for (StockBaseInfo bean : lstResult) {
			...
			if (new BigDecimal(bean.getZsz()).compareTo(new BigDecimal("20000000000")) < 0) {
				if (bean.getPe() <= 20&&bean.getNpe()<=20)
					lstResRet.add(bean);
			} else {
				// 新和成
				//if (bean.getPe() <= 32)
	...
	public static Map<String, String> getAllChkStock() {
		...
		String []sss =  whStock.split(",");
		for (int i = 0; i < sss.length; i++) {
			mapSource.put(sss[i], "whStock");
		}
